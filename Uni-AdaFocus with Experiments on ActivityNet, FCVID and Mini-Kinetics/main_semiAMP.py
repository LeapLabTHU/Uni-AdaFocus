...
from models.uni_adafocus_semiAMP import AdaFocus
...
def main_worker(ngpus_per_node, args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, scaler, criterion, optimizer, epoch, args, log_training)

        if epoch > args.epochs - 10 or (epoch % 1 == 0):
...
def softmax_and_mean(logits, num_channels, num_classes):
    logits = logits.view(-1, num_channels, num_classes)
    soft = nn.Softmax(dim=2)
    logit = soft(logits).mean(dim=1)
    return logit
...
def validate(val_loader, model, criterion, args, log=None):
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            p1 = model(images)
            cat_logits, cat_pred, global_logits, local_logits, temporal_logits, spatial_logits, w = p1
            ...
            cat_logits = softmax_and_mean(cat_logits, args.num_glance_segments + args.num_focus_segments,
                                          args.num_classes)
            global_logits = softmax_and_mean(global_logits, args.num_glance_segments, args.num_classes)
            local_logits = softmax_and_mean(local_logits, args.num_focus_segments, args.num_classes)

            all_cat_result.append(cat_logits)
            all_global_result.append(global_logits)
            all_local_result.append(local_logits)
            all_temporal_result.append(temporal_logits)
            all_spatial_result.append(spatial_logits)
        ...
        all_temporal_result = cat_and_cpu_and_gather(all_temporal_result, args)
        all_spatial_result = cat_and_cpu_and_gather(all_spatial_result, args)
        ...
        if args.rank == 0:
            ...
            if args.dataset == 'minik':
                logits_list = [all_result, all_cat_result, all_global_result, all_local_result, all_temporal_result, all_spatial_result]
                metric_name_list = ['pred', 'cat', 'global', 'local', 'temporal', 'spatial']
                all_targets = all_targets[:, 0]  # for acc calculate
                for i in range(6):
                    name = metric_name_list[i]
                    acc1, acc5 = accuracy(logits_list[i], all_targets, topk=(1, 5))
                    output = output + ('Acc_{name}@1: {top1:.3f} Acc_{name}@5: {top5:.3f} '
                                       .format(name=name, top1=acc1[0], top5=acc5[0]))
            else:
                ...
                temporal_mAP, _ = cal_map(all_temporal_result, all_targets)
                spatial_mAP, _ = cal_map(all_spatial_result, all_targets)
                output = output + ('mAP: {mAP} smAP: {smAP} Cat_mAP: {Cat_mAP} Global_mAP: {Global_mAP} Local_mAP: {Local_mAP} '
                               'Temporal_mAP: {Temporal_mAP} Spatial_mAP: {Spatial_mAP}'
                               .format(mAP=mAP, smAP=smAP, Cat_mAP=cat_mAP, Global_mAP=global_mAP, Local_mAP=local_mAP,
                                       Temporal_mAP=temporal_mAP, Spatial_mAP=spatial_mAP))
...
def train(train_loader, model, scaler, criterion, optimizer, epoch, args, log=None):
    batch_time = AverageMeter('Time', ':6.3f')
    ...
    metric_name_list = ['pred', 'cat', 'global', 'local', 'temporal', 'spatial']
    top1_list = [AverageMeter(f'Acc_{name}@1') for name in metric_name_list]
    top5_list = [AverageMeter(f'Acc_{name}@5') for name in metric_name_list]
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        num_channels_list = [1, args.num_glance_segments + args.num_focus_segments, args.num_glance_segments,
                             args.num_focus_segments, 1, 1]
        logits_list_1 = [pred_1, outputs_1, outputs_global_1, outputs_local_1, outputs_temporal_1, outputs_spatial_1]
        for j in range(1, 4):
            logits_list_1[j] = softmax_and_mean(logits_list_1[j], num_channels_list[j], args.num_classes)
        for a_logit, top1, top5 in zip(logits_list_1, top1_list, top5_list):
            prec1, prec5 = accuracy(a_logit, target, topk=(1, 5))
        ...
        if i % args.print_freq == 0 and args.rank == 0:
            output = ('Epoch: [{0}][{1}/{2}], lr: {lr:.5f}\t'
                      ...
                      'Norm Loss {loss_norm.val:.4f} ({loss_norm.avg:.4f})\t'.format(
                epoch, i, len(train_loader), batch_time=batch_time, data_time=data_time,
                loss=losses, loss_g=losses_g, loss_l=losses_l, loss_t=losses_t, loss_s=losses_s, loss_KL=losses_KL,
                loss_norm=losses_norm, lr=optimizer.param_groups[-1]['lr']))
            for i in range(6):
                output = output + '{top1.name} {top1.val:.3f} ({top1.avg:.3f})\t'.format(top1=top1_list[i])
                output = output + '{top5.name} {top5.val:.3f} ({top5.avg:.3f})\t'.format(top5=top5_list[i])
            output = output + 'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\tPrec@5 {top5.val:.3f} ({top5.avg:.3f})\t'.format(
