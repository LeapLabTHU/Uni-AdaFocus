...
class TSNDataSet(data.Dataset):
    ...
    def _sample_indices(self, record):
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            offsets = np.random.rand(self.num_segments) + np.arange(self.num_segments)
            offsets = offsets / self.num_segments * record.num_frames
    ...
    def _get_val_indices(self, record):
        if self.dense_sample:  # i3d dense sample
            ...
            offsets = np.ones(self.num_segments) * 0.5 + np.arange(self.num_segments)
            offsets = offsets / self.num_segments * record.num_frames
            offsets = np.floor(offsets)
            return offsets + 1
