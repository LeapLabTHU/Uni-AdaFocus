...
# Code is adapted from https://github.com/facebookresearch/pytorchvideo/blob/main/pytorchvideo/models/x3d.py
...
def create_x3d(
    *,
    ...
    blocks.append(head)
    return NetWithMap(blocks=nn.ModuleList(blocks))
...
class NetWithMap(nn.Module):
    ...
    def get_featmap(self, x: torch.Tensor):
        ...
        if head.pool is not None:
            ...
            pool = head.pool
            x = pool.pre_conv(x)
            if pool.pre_norm is not None:
                x = pool.pre_norm(x)
            if pool.pre_act is not None:
                x = pool.pre_act(x)

            x1 = pool.pool(x)  # [B, C, T, H, W] -> [B, C, 1, 1, 1]
            x1 = pool.post_conv(x1)
            if pool.post_norm is not None:
                x1 = pool.post_norm(x1)
            if pool.post_act is not None:
                x1 = pool.post_act(x1)
        ...
        return x1.flatten(1), x, x.mean([3, 4])
