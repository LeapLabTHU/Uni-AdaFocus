...
We explore the phenomenon of **spatial redundancy**, **temporal redundancy** and **sample-wise redundancy** in video understanding and propose Uni-AdaFocus, an efficient end-to-end video recognition model.
Uni-AdaFocus is built on top of AdaFocus, which uses a lightweight encoder and policy network to identify and process the most informative spatial regions in video frames. 
Uni-AdaFocus extends AdaFocus by dynamically allocating computation to the most task-relevant frames and minimizing computation resource on easier videos. Uni-AdaFocus is compatible with off-the-shelf efficient backbones (e.g. TSM and X3D), and can further improve their inference efficiency. 
Extensive experiments on seven benchmark datasets (i.e, ActivityNet, FCVID, Mini-Kinetics, Something-Something V1&V2, Jester, and Kinetics-400) and three real-world application scenarios (i.e, fine-grained diving action classification, Alzheimer's and Parkinson's diseases diagnosis with brain magnetic resonance images (MRl), and violence recognition for online videos) substantiate that Uni-AdaFocus is considerably more efficient than the competitive baselines.
...
For reproducing our experimental results, please go to following folder for specific instructions:
...
For applying Uni-AdaFocus to customized downstream tasks, check this tutorial:
